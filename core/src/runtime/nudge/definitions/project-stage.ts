...
const COLLECT_FULL_TEXT = [
  ...
  "2. 故事构建**建议先行**：基于创意直接给出你的构建方案（主角/冲突/基调/书名等，并列 2-3 个候选方向），作者确认或修正后定稿；不要逐项提问采集，可自行推断的（如常规写法组合）给推荐即可。",
...
const OUTLINE_FULL_TEXT = [
  ...
  "动笔细化前查 <memory> 目录（见「记忆偏好案例库」）：story 池（未用优先）可直接采用进大纲，采用即标记；需要编排参考时用 NovelRead 查本项目既有单元。",
...
const PROSE_FULL_TEXT = [
  ...
  "2. 动笔前查 <memory> 目录（见「记忆偏好案例库」）：Read 同类目 prose 案例对照质感与节奏（不复用其词句）。对照「正文规范」成文；正文草稿用 ```novel 代码块呈现；引用实体用标签（<character id=\"...\"> 等）。",
