 ...
 * 供 ManifestSystemPromptCompiler 对齐验证（ccb_main_reference 冒烟）。
 * Assembles CCB main-agent static system prompt sections in original order for
 * ManifestSystemPromptCompiler alignment validation (ccb_main_reference smoke).
 *
 * 说明 / Notes：
 ...
 *   requiredSectionIds: [] 调用 ManifestSystemPromptCompiler，否则会触发必选段校验失败。
