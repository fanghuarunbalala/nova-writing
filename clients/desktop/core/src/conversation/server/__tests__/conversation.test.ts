...
describe("ConversationManagerServer", () => {
  ...
  it("根会话 bypass：canonical 写直接批准（入队即决议 + 直推回传，根完全自主）", async () => {
    ...
    await conv.sendSystemControl({ type: "mode.set", mode: "bypass" });
    await conv.promotePendingMode();
    const pending = conv.sendApprovalRequest({ requestId: "r1", toolCalls: [{ toolCallId: "t1", toolName: "NovelWrite", args: "{}" }] });
    await server.submitApprovalRequest(ref.conversationId, { requestId: "r1", toolCalls: [{ toolCallId: "t1", toolName: "NovelWrite", args: "{}" }] });
    expect(await pending).toEqual({ kind: "approve" });
