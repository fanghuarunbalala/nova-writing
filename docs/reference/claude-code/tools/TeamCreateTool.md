...
- **工具名**: `TeamCreate`（userFacingName: `''`）
- **源码**: `vendor/claude-code/packages/builtin-tools/src/tools/TeamCreateTool/`
- **门槛**: 无条件（getAllBaseTools 直接列出）；`isEnabled()`: `true`

## 描述（模型侧 desc）

来源：`TeamCreateTool.ts` 内联。`description()` 返回：
...
- `team_name` (string, 必填): "Name for the new team to create."
- `description` (string, 可选): "Team description/purpose."
- `agent_type` (string, 可选): "Type/role of the team lead (e.g., \"researcher\", \"test-runner\"). Used for team file and inter-agent coordination."

## 附加模型侧内容

`prompt()` 返回 `prompt.ts` 中 `getPrompt()` 的团队协作指令（原文）：
