...
- **工具名**: `ExecuteExtraTool`（userFacingName: `ExecuteExtraTool`）
- **源码**: `vendor/claude-code/packages/builtin-tools/src/tools/ExecuteTool/`
- **门槛**: 无条件（`getAllBaseTools` 末尾；源码注释：first-class tool——always available，非 deferred。模型经 SearchExtraTools 发现 deferred 工具后用本工具执行）
- **性质**: isConcurrencySafe: `false`
...
- `tool_name` (string, 必填): "The exact name of the target tool to execute (e.g., \"CronCreate\", \"mcp__server__action\")"
- `params` (record<string, unknown>, 必填): "The parameters to pass to the target tool"
