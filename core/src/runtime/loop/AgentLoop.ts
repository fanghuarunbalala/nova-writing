...
export class AgentLoop {
  ...
  /** 审批批次序号（requestId 尾段 b{n}：同 turn 多轮工具批次不撞队列幂等） */
  private batchSeq = 0;
  ...
  private async runTurnLoop(
    ...
    for (runContext.curTurn = 0; runContext.curTurn < runContext.maxTurn; runContext.curTurn++) {
      ...
      if (result.finishReason === "tool_call" && result.message.toolCalls) {
        ...
        const gate = await this.gateBatch(result.message.toolCalls, turn.seq);
   ...
   */
  private async gateBatch(toolCalls: readonly ToolCall[], turnSeq: number): Promise<Map<string, string>> {
    ...
    const decision = await this.config.requestApproval({
      requestId: `approval:${this.config.conversationId ?? "conv"}:${turnSeq}:b${++this.batchSeq}`,
