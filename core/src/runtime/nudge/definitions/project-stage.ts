...
const COLLECT_FULL_TEXT = [
  ...
  "1. 通过 AskUserQuestion 获取一句话创意（开放填空，绝不配选项）与全篇目标字数及每章推荐字数（冷启动不带「推荐」——推荐必须有前文依据）。规划可逐步展开：先固化故事核与创作范围（如先定第一卷），其余后续再说，不要求一次定完全书。",
...
const GLOBAL_RULES_FOOTER = [
  ...
  "- 逐步展开：规划不要求一次定完；作者指定先做范围（如先确定第一卷）时以此为准，其余后续再说。",
