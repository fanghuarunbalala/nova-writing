...
const OUTLINE_FULL_TEXT = [
  ...
  "2. 取一个候选（最靠前或作者指定），通过 AskUserQuestion 协作拆分：每个子故事一句话核心事件＋情绪走向；检查四种串联（因果/情绪对比/悬念牵引/目标递进）。",
...
const PROSE_FULL_TEXT = [
  ...
  "1. 通过 NovelRead(kind=story_unit, includePlans=true) 按书序定位第一个未完成故事，读其 5 要素与前后故事的情绪标签。",
...
const ROADMAP_FOOTER = [
  ...
  "- 开书推荐工作流：项目尚无任何内容 —— 采集一句话创意与字数设定，构建并固化故事核（NOVEL.md 或正式稿）。",
  "- 大纲推荐工作流：已有故事核或故事单元 —— 逐个拆分到不可再分并补全 5 要素。",
  "- 正文推荐工作流：已有不可再分且 5 要素完整的故事 —— 按书序逐故事成文。",
...
export function renderSparseText(stage: ProjectStage, action: ProjectStageAction): string {
  if (action.workflow === "collect") {
    return "【项目状态】开书 · 项目尚无任何内容 · 下一步：从一句话创意开始，采集设定并构建故事核";
  }
  const s = stage.stats;
