# 项目域上云-纯云端项目 PRD —— v0.2

> 状态：✅ 期 1 已实施（feat/cloud-projects：FR1-FR6 落地，契约冻结 `protocol/cloud-project-api.md`；期 1 内实现取舍见 §4 注与 §7）
...
- [x] 登录后新建云项目只输入名字；最近列表云端分区来自 server；本地分区与现状一致
- [x] 云项目会话：Read/Glob/Write/Edit 读写 server 文件（ProjectFiles port + RemoteProjectFiles）；NOVEL.md 仍只能经审批落盘；memory 保留 source 校验
- [x] 沙箱：逃逸/绝对/盘符/超限/黑名单路径在 server 端被拒（files.test.ts 用例表），与桌面沙盒对拍一致（files-parity.test.ts；对拍中修掉桌面 UNC 漏网）
- [x] 域链路：snapshot 首拉、mutate 乐观锁 409 自纠、SSE domain_changed 他端可见（cloud-domain.test.ts / cloud-novel-store.test.ts 真 server 集成）
- [x] 双端一致性：桌面写 →（第二会话）delta 重放读到同一状态（cloud-novel-store 集成测试）
- [x] 回归：core 957 / server 69 / ui 468 / gui tsc 0 全绿；本地项目与未登录路径零改动通过
- [x] Android M4 复用：契约冻结 `protocol/cloud-project-api.md`（接口形状/沙箱规则/oplog 语义）

> 期 1 实现取舍（诚实记录）：FR6 的 RemoteNovelStore 采用**投影+oplog**架构（InMemoryNovelStore
> 全量域语义做本地投影、mutation 以 oplog 形态入 domain_entities、seq 全序重放收敛），而非把域
> 引擎重写在 server——server 保持纯数据面。跨端并发写保护 = 会话租约互斥；乐观锁自纠发生在投影
> 引擎内（stale baseRevision 拒）。云项目的本地缓存目录仅承担 journal sidecar/设计稿缓存等兜底，
> 非权威数据。
