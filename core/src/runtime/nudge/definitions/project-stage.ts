...
const COLLECT_FULL_TEXT = [
  "## 开书推荐工作流：从一句话到吸引人的故事",
  "入口：小说项目当前尚无任何内容。",
  "1. 通过 AskUserQuestion 获取一句话创意（开放填空，绝不配选项）与全篇目标字数及每章推荐字数（冷启动不带「推荐」——推荐必须有前文依据）。",
  "2. 通过 AskUserQuestion 帮助用户完成一个吸引人的故事构建。",
  "3. 作者确认后的内容按要求写入 NOVEL.md 或者正式稿。",
].join("\n");

const OUTLINE_FULL_TEXT = [
  "## 工作流·大纲：逐步细化（故事与场景同构）",
  "入口：NOVEL.md 已有故事核，或已存在故事单元。",
  "核心：一个故事能拆成 ≥2 个各自有独立情绪转折的块就继续拆；拆不动 = 不可再分 —— 推进 planningStatus 到 ready 并补全 leaf 5 要素（人物/地点/事件/转折/情绪）。",
  "1. NovelRead(kind=story_unit, includePlans=true) 识别各故事粒度：不可再分且 5 要素全 → 跳过；不可再分缺要素 → 补全；其余 → 候选拆分。",
  "2. 取一个候选（最靠前或作者指定），AskUserQuestion 协作拆分：每个子故事一句话核心事件＋情绪走向；检查四种串联（因果/情绪对比/悬念牵引/目标递进）。",
  "3. 子故事可再拆 → 标记待拆下轮处理；不可再拆 → 补全 5 要素。",
  "4. 每轮检查：相邻串联成立、局部情绪曲线无连续 3 个同强度、无「什么不是好的大纲」任一特征（对照「大纲规范」）。",
  "5. 向作者展示当前大纲，请求确认或指定下一个细化对象。",
  "可重复执行：每次一个或一批故事，不要求一次细化完。",
  "完成标准（单个故事）：不可再分 ＋ 5 要素全 ＋ 前后串联验证。完成标准（整体）：全部如此、整体情绪曲线有波动、无坏特征。",
  "种子生长：细化中发现故事核需补充（如加配角承载冲突线）直接补进 NOVEL.md 并告知，不视为回退。",
  "结构调整量极大时可 EnterComposeMode 批量出方案（可选，非必经）。",
].join("\n");

const PROSE_FULL_TEXT = [
  "## 工作流·正文：为不可再分的故事成文",
  "入口：至少一个故事不可再分且 5 要素完整（不要求全部细化完）。",
  "1. 按书序定位第一个未完成故事（NovelRead kind=story_unit, includePlans=true 看 progress），读其 5 要素与前后故事的情绪标签。",
  "2. 对照「正文规范」成文；正文草稿用 ```novel 代码块呈现；引用实体用标签（<character id=\"...\"> 等）。",
  "3. 作者审阅三态：通过 → 定稿标 completed；小改（措辞/节奏/细节）→ 当场改再提交；大改（方向/情绪/事件逻辑）→ 改该故事要素后重新成文。最多两轮，仍不满 → 标 blocked，跳下一个故事。",
  "4. 定稿后看下一个故事：已细化 → 继续写；未细化 → 先做一轮「大纲·逐步细化」再回来。",
  "发布层延后：写作中不切章；每完成 10-15 个故事统一组装一次（按情绪节点切章、大弧线归卷），组装结果可调整。",
  "降级：要素小问题就地补全；要素大问题改后重写；故事核根本问题 = 唯一重跑「开书」的情况，必须告知作者原因。",
].join("\n");
...
const ROADMAP_FOOTER = [
  "## 工作流路线图（全文在各自阶段开始时注入一次）",
  "- 开书：尚无故事单元 —— 采集创意并固化 NOVEL.md。",
  "- 大纲·逐步细化：存在未细化故事 —— 逐个拆分到不可再分并补 5 要素。",
  "- 正文：存在已细化未写的故事 —— 逐故事成文。",
  "- 收尾：全部故事完成 —— 发布组装与一致性检查。",
].join("\n");
...
const GLOBAL_RULES_FOOTER = [
  "## 全局规则",
  "- 不重复问：作者已给的信息不再询问。",
  "- 不阻塞：作者拒绝建议时记录决定、标注风险，继续推进。",
  "- 不静默回退：需要重跑上一阶段必须告知作者原因。",
  "- 种子生长不是回退：在后续阶段补充前序信息（含直接修正并告知）。",
...
export function renderSparseText(stage: ProjectStage, action: ProjectStageAction): string {
  if (action.workflow === "collect") {
    return "【项目状态】开书 · 尚无故事单元 · 下一步：采集并固化故事核（NOVEL.md），或建首个故事进入大纲细化";
  }
  const s = stage.stats;
  if (action.workflow === "expand_outline") {
    return `【项目状态】大纲细化 · 故事 ${s.completedCount}/${s.totalScenes} 完成、${s.refinedCount}/${s.totalScenes} 不可再分 · 下一步：细化「${action.target?.title ?? "第一个未细化故事"}」（拆分到不可再分并补 5 要素）`;
  }
  if (action.workflow === "write_prose") {
    return `【项目状态】撰写正文 · 故事 ${s.completedCount}/${s.totalScenes} 完成 · 下一步：写「${action.target?.title ?? "第一个未写故事"}」`;
  }
  return `【项目状态】收尾 · 全部 ${s.totalScenes} 个故事完成 · 下一步：发布组装与一致性检查`;
}
