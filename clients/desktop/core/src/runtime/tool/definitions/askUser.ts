...
function toQuestionSpec(raw: unknown, index: number): AskQuestionSpec {
	...
	// options 与 placeholder 并用：降级不拒绝——选择题 UI 本就自动提供「其他」自由输入，
	// 忽略 placeholder 语义无损（硬拒绝会浪费一轮 provider 重试；conv_1 实测模型爱写
	// 「书名（自由发挥）」这类候选+自由发挥组合，属自然用法）
...
const ASK_USER_QUESTION_DESCRIPTION = [
  ...
  "6. 动笔/委派前的设计点确认：委派 Compose 或进入设计模式前，范围、粒度、情绪走向等返工成本高的设计点——先问清再出稿，避免整块返工（一次 ≤2 问，可给 2-4 个候选）。",
  "",
  "## 何时不用",
  ...
  "6. 能基于既有设定/正文/工作流推断出合理方案的——先给建议方案并说明假设，不要用提问把功课推给作者（返工成本高的设计点确认除外：有推断方案也要先问清再出稿）。",
  "",
  "## 示例",
  "<example>",
  "作者：开始写第二卷吧",
  ...
  "- 开放填空题（一句话创意、既有构思等）只有一个文本框：省略 options，**绝不配选项**——候选会把作者思路锚死；placeholder 是纯文本输入提示，选择题不需要（UI 自动提供自由输入）。",
  "- multiSelect: true 用于素材/伏笔/支线取舍等选项不互斥的场景。",
  "- 返回作者的逐问回答（选择/自填/跳过）；跳过 = 作者授权你自行决断，不要重复追问。",
  "",
  "先查再问；能查证的先查证，返工成本高的设计点必须先问清再动笔；问了就按答案走，不重复问。",
].join("\n");
