...
1. **持久化会话**——会话状态与 journal 持久化，崩溃后可恢复
2. **Subagent：Explore / Compose**——只读探索与草案创作委托
3. **Nudge 机制**——AI 主动向作者提问、采集创意
4. **三档会话模式**——review（审核）/ bypass（直接运行）/ compose（设计：先想清楚写什么，再入库）
5. **上下文压缩**——三道门禁 + 超窗保险丝
6. **GUI 界面**——整体界面已大体落地（审批清单、提问卡、目录下钻等）
7. **Skills 技能系统**——兼容 Agent Skills 开放标准（`npx skills add` 可直接安装），技能索引渐进披露、按需读取，设置页可管理当前生效技能
8. **MCP 工具接入**——设置页管理 MCP 服务器（stdio / http、测试连接、信任免审），外部工具默认进审批清单，新会话生效

**后续计划**：

- **导入正在创作的内容**——把已有作品导入工作台
- **自动化与 teammate**——定时执行 teammate agents，作为团队 agents 完成草稿，并交由你审批
- **评测 harness**——给「AI 写得好不好」一套可量化的答案，而不是全凭感觉
- **memory 机制**——跨会话的记忆体系
- **伏笔系统**——伏笔的埋设、跟踪与回收管理，让前后照应有据可查
