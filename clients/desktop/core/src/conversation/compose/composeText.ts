...
export function renderComposeModeFullText(designFilePath?: string): string {
	return [
		...
		"",
"### Phase 1: 理解需求",
"聚焦给定的创作需求，阅读相关既有设定（大纲/人物/地点），理解当前故事结构与约束。若设计点（目标/范围/要点）未确定或存在分歧，先用 AskUserQuestion 与作者确认——设计点未完全确定前不进入草案创作。",
		"",
		"### Phase 2: 探索",
		"建议派 **Explore** 子代理并行查设定、时间线、伏笔、矛盾点；复杂任务必派，琐碎任务可直接用只读工具自行探索。",
		"",
"### Phase 3: 创作草案",
"默认派 **Compose** 子代理设计大纲或正文草案（委派 prompt 写明目标、范围与要点，只包含已与作者确认的设计点；一次只委派一个已确认的块，不打包）；琐碎草稿可自行创作。",
		"",
		"### Phase 4: 综合写入草稿",
		"评审子代理产出，用 Write/Edit 增量完善 design 文件（唯一可写文件）。",
		"",
"### Phase 5: 提交审批",
"提交前自查：草稿是否全部基于已与作者确认的设计点——含未确认的大块设计先拆分问清再提交。草稿完成后调用 **ExitComposeMode** 提交审批；不得用文本询问审批；若被拒：按反馈修订后重新提交，不要原样重试。",
	].join("\n");
}
