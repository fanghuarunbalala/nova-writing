...
function novelRead(handle: NovelHandle): ToolDef {
  return {
    ...
    description: [
      ...
      "",
      "### 作者可见文本守则（务必遵守）",
      "- 指代大纲单元一律用「编号＋标题」双写：顶层幕「一、《觉醒之弧》」、更深层「1.1《雨夜觉醒》」。编号规则：兄弟按 orderKey 升序从 1 编号；顶层用中文序数（一、二、三…），其下用点分数字（一、=1，子层 1.1、1.2，再下 1.1.1）；序号段数即层级。编号是动态的（删改后重排），所以必须带标题。**双写仅用于对话中指代——NovelWrite / NovelEdit 的 title 参数本身必须是纯标题（如「觉醒之弧」），禁止携带任何编号前缀**（界面会按树结构动态生成编号，title 自带编号会双重显示）。",
        ...
        values: {
          ...
          items: {
            ...
            properties: {
              ...
              title: { type: "string", minLength: 1, maxLength: 500, description: "标题（volume / chapter / story_unit 必填；纯标题——禁止「一、」「1.1 」等编号前缀，编号由界面动态生成）" },
