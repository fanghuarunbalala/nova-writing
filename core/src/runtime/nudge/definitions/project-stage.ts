...
const COLLECT_FULL_TEXT = [
  ...
  "1. 一句话创意与全篇目标字数及每章推荐字数只存在于作者脑中——用一次 AskUserQuestion（≤2 问）采集：一句话创意（开放填空，绝不配选项）＋目标篇幅（冷启动不带「推荐」——推荐必须有前文依据）。规划可逐步展开：先固化故事核与创作范围（如先定第一卷），其余后续再说，不要求一次定完全书。",
  "2. 故事构建**建议先行**：基于创意直接给出你的构建方案（主角/冲突/基调/书名等，并列 2-3 个候选方向），作者确认或修正后定稿；不要逐项提问采集，可自行推断的（如常规写法组合）给推荐即可。候选方向可派**草案创作（Compose）子代理**并行起草，你汇总修订后呈现（与作者的问答互动必须由你本人完成）。",
...
const OUTLINE_FULL_TEXT = [
  ...
  "1. 通过 NovelRead(kind=story_unit, includePlans=true) 识别各故事粒度：不可再分且 5 要素全 → 跳过；不可再分缺要素 → 补全；其余 → 候选拆分。",
  "2. 取一个候选（最靠前或作者指定），**默认经设计模式生成**：EnterComposeMode 后派 **草案创作（Compose）子代理**出拆分方案（委派 prompt 写明目标故事、细化范围与串联要求），你审阅修订后写入设计草稿，ExitComposeMode 经作者审批应用。方案形态：每个子故事一句话核心事件＋情绪走向；检查四种串联（因果/情绪对比/悬念牵引/目标递进）。方向性分歧或跨卷大结构先用 AskUserQuestion 确认再进入设计模式；零星小调整（如补单个要素）可不经设计模式直接改。",
...
const PROSE_FULL_TEXT = [
  ...
  "1. 通过 NovelRead(kind=story_unit, includePlans=true) 按书序（或作者指定范围，如第一卷内）定位第一个未完成故事，读其 5 要素与前后故事的情绪标签。",
  ...
  "3. 作者审阅三态（设计模式下对应 ExitComposeMode 审批决议）：通过 → 定稿标 completed；小改（措辞/节奏/细节）→ 当场改再提交；大改（方向/情绪/事件逻辑）→ 改该故事要素后重新成文。最多两轮，仍不满 → 标 blocked，跳下一个故事。",
...
const ROADMAP_FOOTER = [
  ...
  "- 开书推荐工作流：尚无已确认故事核 —— 采集创意与设定，构建并固化故事核（NOVEL.md 或正式稿）。",
