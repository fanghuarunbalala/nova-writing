...
export class OpenAIProvider extends BaseProvider {
  ...
  /**
   * 上游请求流（待接入 OpenAI SDK 后实现：转译中立请求 → SDK 格式，迭代上游流）
   * @param call 单次请求参数
   * @returns 原始 chunk 迭代
   */
  protected override async *createStream(call: ProviderCall): AsyncIterable<unknown> {
    void call;
    throw new Error("OpenAIProvider.createStream 尚未实现");
  }
  ...
  protected override normalizeDelta(chunk: unknown): ProviderDelta | null {
    void chunk;
    throw new Error("OpenAIProvider.normalizeDelta 尚未实现");
  }
  ...
  protected override buildResult(call: ProviderCall): ProviderResult {
    void call;
    throw new Error("OpenAIProvider.buildResult 尚未实现");
  }
}
