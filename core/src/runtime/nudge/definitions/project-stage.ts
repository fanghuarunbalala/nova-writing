...
const COLLECT_FULL_TEXT = [
  ...
  "2. **默认经设计模式分节构思**：EnterComposeMode 后把故事构建拆成固定小节逐节推进——① 主角（性格/身份/金手指）② 灵魂设定（创意核心意象）③ 世界观与力量体系 ④ 基调与书名 ⑤ 故事核汇总（NOVEL.md 固化清单）。",
  "3. 每节派**草案创作（Compose）子代理**出候选：一次只委派当前小节，prompt 只要求该节内容、**并附此前各节已确认的结果摘要**；该节存在未定设计点（如主角性格取向、世界观风格）时先 AskUserQuestion 问清再委派；节内可并列 2-3 个候选，委派时可从「本工作流参考案例」为当前小节点名对应案例。你审阅修订后写入设计草稿并**只呈现当前一节**，作者确认或修正后才进入下一节——**绝不一次输出成套方案**；与作者的问答互动必须由你本人完成。",
  "4. 第 ⑤ 节故事核汇总不委派出候选：由你整合前四节已确认内容成固化清单（可派 Compose 仅做格式润色）。",
  "5. 五节逐节确认完毕后 ExitComposeMode 提交审批；经作者批准后，故事核固化清单按要求写入 NOVEL.md 或者正式稿。",
].join("\n");
...
const OUTLINE_FULL_TEXT = [
  ...
  "2. 取一个候选（最靠前或作者指定），**委派前先用 AskUserQuestion 明确拆分设计点**：拆分粒度（拆到 arc/sequence/scene 哪层）、本次范围（一个故事或作者指定范围）、各子故事情绪走向与串联方式；设计点完全确定后再 EnterComposeMode，派**草案创作（Compose）子代理**出拆分方案（委派 prompt 只写已确认设计点：目标故事、细化范围与串联要求），你审阅修订后写入设计草稿，ExitComposeMode 经作者审批应用。**一次只委派已确认的块，不打包整卷/整批出方案**。方案形态：每个子故事一句话核心事件＋情绪走向；检查四种串联（因果/情绪对比/悬念牵引/目标递进）。零星小调整（如补单个要素）可直接改。",
...
const PROSE_FULL_TEXT = [
  ...
  "2. **默认经设计模式生成**：**委派前先用 AskUserQuestion 明确成文设计点**：视角与基调、篇幅与节奏、情绪落点、开篇钩子；确定后 EnterComposeMode 派**草案创作（Compose）子代理**成文（委派 prompt 只写已确认设计点：目标故事 5 要素、情绪走向、节奏拍分布与前后衔接），你对照「正文规范」审阅修订后写入设计草稿，并以段落 intensity 自查情绪曲线——**无连续 3 个同强度、峰值成片不孤立**；ExitComposeMode 经作者审批后写入正式稿定稿：NovelWrite(kind=paragraph) 逐句写入（对话中文双引号、每句一段）并**逐段标注 rhythm/intensity（必填）**。**一次一个故事，不批量委派多篇**。正文用 ```novel 代码块呈现；引用实体用标签（<character id=\"...\"> 等）。小改重写（措辞/节奏/细节）可不经设计模式直接改。",
