...
const OUTLINE_FULL_TEXT = [
  ...
  "核心：两级细化，按作者意愿推进——幕级拆分（story unit → story unit）：按作者指定范围扩展——横向拆一层（1 → 1.1/1.2/1.3）或纵向深入（1 → 1.1 → 1.1.1）直到场景（leaf）；场景级补全（story unit → leaf scene）：场景是最小单元，是故事核（时间/地点/人物、事件序列、节奏拍、状态变更），按三阶段确认。不要求一次细化完，想到哪细化到哪；**每次扩展前先询问作者本次范围**。",
  ...
  "3. 逐层拆到场景（leaf）为止，**一次只委派当前一幕，不打包整卷/整批出方案**；作者确认后 ExitComposeMode 提交审批：通过则实施，驳回则等待作者补充信息后重来。",
  "### 二、场景细化（story unit → leaf scene）",
  ...
  "展示：向作者展示当前大纲（用 saga/arc/sequence/scene 层级表述，不用「第X章」，见「大纲规范·概念边界」），说明本轮成果。",
  "可重复执行：按作者指定范围扩展——横向拆一层或纵向深入任一子块均可，不要求一次细化完，想到哪细化到哪；扩展范围每次由作者指定，**扩展前先询问确认**。",
  "完成标准：只要有一个可用的 LeafPlan 即视为完成。",
  "批量调整：批量细化与跨卷结构调整同样走设计模式经子代理出方案（设计模式内流程见进入时的注入全文）。",
].join("\n");

const PROSE_FULL_TEXT = [
  "## 正文推荐工作流：为 LeafPlan 已定的场景成文",
  "入口：至少一个场景（leaf）已有完整 LeafPlan（不要求全部细化完）。",
  "1. 经 NovelRead(kind=story_unit, includePlans=true) 按书序（或作者指定范围，如第一卷内）定位第一个未完成场景，读其 LeafPlan（时间/地点/人物/事件序列/节奏拍）与前后场景的情绪标签。",
  "2. 进入 EnterComposeMode（设计模式），按 LeafPlan 事件序列逐步推进，除非用户明确要求，否则每个事件都要求作者确认，确认后才推进下一个事件。对当前事件：",
  ...
  "3. 当前 LeafPlan 完成后 ExitComposeMode 提交审批：通过则执行写入——NovelWrite(kind=paragraph) 逐句写入（对话中文双引号、每句一段）、**逐段标注 rhythm/intensity（必填）**，并定稿标 completed；驳回则按用户要求继续修改。",
  "发布层延后：写作中不切章；每完成 10-15 个场景统一组装一次（按情绪节点切章、大弧线归卷），组装结果可调整。",
  "降级：LeafPlan 小问题就地补全；大问题改后重写；故事核根本问题 = 唯一重跑「开书推荐工作流」的情况，必须告知作者原因。",
...
const ROADMAP_FOOTER = [
  ...
  "- 大纲推荐工作流：已有故事核或故事单元 —— 按作者指定范围逐幕细化：幕级拆分到场景（leaf），场景按三阶段（时间地点人物 / 事件序列节奏拍 / 状态变更）补全 LeafPlan（不要求一次规划完全书）。",
  "- 正文推荐工作流：已有 LeafPlan 完整的场景 —— 按书序（或作者指定范围）逐场景成文。",
...
const GLOBAL_RULES_FOOTER = [
  ...
  "- 不硬写：没有完整 LeafPlan 不强行成文；「足够」= LeafPlan 完整而非完美。",
...
export function renderSparseText(stage: ProjectStage, action: ProjectStageAction): string {
  ...
  if (action.workflow === "expand_outline") {
    return `【项目状态】大纲细化 · 故事 ${s.refinedCount}/${s.totalScenes} 不可再分、${s.completedCount}/${s.totalScenes} 正文完成 · 下一步：细化「${action.target?.title ?? "第一个未细化故事"}」（细化到场景（leaf）并补全 LeafPlan）`;
  }
