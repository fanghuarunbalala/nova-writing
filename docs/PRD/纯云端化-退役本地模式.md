# 纯云端化-退役本地模式 PRD —— v0.2

> 状态：✅ 已实施（feat/cloud-projects：FR1-FR8 落地 + 会话首开提速两增强——回放竞态修复/预播种、历史分段懒加载；实现取舍见 §4 注与 §6）
...
> 契约：`protocol/cloud-project-api.md` v1.1（replay?since= / history 分页 / UI 域通道 / 缓存与镜像）。
...
- [x] 强制登录：未登录无法进入工作台，全 UI 无「本地模式」入口与文案
- [x] 打开项目 = 云端列表；新建只命名；删除 = server 软删 + registry/缓存清理
- [x] UI 手动域写（建角色/卷章）经 main 内 RemoteNovelStore + `ui-<projectId>` 编辑租约直写 server（cloud-novel-store 集成：无租约拒/可写/与会话租约不互斥/释放后重取自愈）；agent 写后 UI 下次 query 可见（无本地 novel.db 分叉）
- [x] 云会话重开：增量恢复（desktop-contract 断言 replay 带 since 游标 + 镜像零重复）；UI 历史非空（main 预播种，首开即回显）
- [x] 域快照缓存命中后 `init` 只发 delta 请求（remote-novel-store 单测断言调用序列）
- [x] env 断链修复：`applyServerEnv` 双前缀注入 `NOVA_*`（云分支激活）
- [x] 会话首开提速（实施中追加的两增强）：①回放竞态修复（appendMirrorRows 按 gs 追加时点去重 + 内存尾游标）；②历史分段懒加载（首开只载最近 50 run + 滚顶加载更早 + 打开项目后台预播种最近 5 会话预热折叠缓存）
- [x] 回归：core 973 / server 73 / ui 457 vitest 全绿 + gui tsc 0（android gradle 不受影响）
- [x] 文档：`protocol/cloud-project-api.md` v1.1（replay `?since=`、env 表、UI 域通道、缓存/镜像、history 分页）；本 PRD 状态更新

> 实现修正记录（对 §4 的两处收紧）：①main 侧 sessionTag 不能用固定 `ui-<projectId>`（重放跳过会在
> 进程重启后丢自身旧操作），落地为 `ui-<projectId>-<pid>`（租约 conversationId 保持稳定 `ui-<projectId>`）；
> ②读侧 history 分页以 `latest/before` 尾部页语义落地（原「fromSeq 前向」对「最近一页」表达力不足），
> fromSeq 前向语义保留给 resume/断档补拉。
