...
> 状态：✅ 已定稿并实现（2026-08-17，v2.0 落地：core 604 用例全绿 + evals 19 全绿 + typecheck 通过；F10 经核实由既有事件词表天然满足——时间线事件仅 user/assistant，system 本就不进 UI，nudge 标记保留为未来守卫；v2.1 2026-08-18：工作流生成环节默认倾向设计模式 + Compose 子代理生成，core 全量回归绿）
...
> 演进：v1.0 瞬态头注（重启/压缩/clear 全量注入）→ v2.0 **persistent append 通道 + full/sparse 双密度 + 逐步注入 + 压缩强制清除 + UI 不展示** → v2.1 **生成环节倾向化**（开书候选可委派 Compose；大纲/正文默认经 EnterComposeMode + Compose 子代理生成，零星小调整保留直改出口；设计模式 Phase 3 收紧为"默认派"）
...
**开书**（作者定稿文案）：通过 AskUserQuestion 获取一句话创意（开放填空，绝不配选项）＋全篇目标字数及每章推荐字数（冷启动不带「推荐」）→ 通过 AskUserQuestion 帮助用户完成一个吸引人的故事构建 → 作者确认后的内容按要求写入 NOVEL.md 或者正式稿。入口=尚无已确认的故事核（项目可能已有零散设定或旧稿）；规划可逐步展开：先固化故事核与创作范围（如先定第一卷），其余后续再说，不要求一次定完全书。候选方向可派 Compose 子代理并行起草，主代理汇总修订后呈现（与作者的问答互动必须主代理本人完成）。
